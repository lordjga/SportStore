    ...
    public static class DbExtension
    {
        public static IHost MigrateDatabase<TContext>(this IHost host)
        {
            using (var scope = host.Services.CreateScope())
            {
                ...
                var context = services.GetRequiredService<IDapperContext>();
                var logger = services.GetRequiredService<ILogger<TContext>>();
                try
                {
                    logger.LogInformation("Discount DB Migration Started");
                    ApplyMigrations(config, context);
        ...
        private static void ApplyMigrations(IConfiguration config, IDapperContext context)
        {
            using var dbConnection = context.CreateNpgsqlConnection();
            dbConnection.Open();
